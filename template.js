;(function($){
	...
	$(document).ready(function(){
		...
		$('.slider-toolbar-carousel .cactus-sub-wrap').each(function(index, element) {
			...
			$this.slick({
				arrows:						true,	
			  	...
			  	infinite: 					true,
			  	speed: 						500,
			  	slidesToShow: 				$fullWidth?(6):($boxItems?3:4),
			  	slidesToScroll:				1,
			  	adaptiveHeight: 			true,
				autoplay:					false,
				autoplaySpeed:				5000,
		...
		$('#video-open-series').on('click', function(){
			var $first_post = $('.post-list-in-single .cactus-post-item:first-child a');
			if($first_post.length > 0) {
				document.location.href=$first_post.attr('href');
			};			
			return false;
		});
